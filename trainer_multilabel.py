    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                mask_lb3 = masks[:, None, 2, :, :].to(self.device)

                predmask_lb1 = pred_masks[:, None, 0, :, :].to(self.device)
                predmask_lb2 = pred_masks[:, None, 1, :, :].to(self.device)
                predmask_lb3 = pred_masks[:, None, 2, :, :].to(self.device)
                ...
                sqNet.zero_grad()
                loss_lb1.backward(retain_graph=True)
                loss_lb2.backward(retain_graph=True)
                loss_lb3.backward()
                optimizer.step()
            ...
            with torch.no_grad():
                ...
                for test_idx, data in enumerate(self.test_dataloader):
                    ...
                    test_mask_lb1 = test_masks[:, None, 0, :, :].to(self.device)
                    test_mask_lb2 = test_masks[:, None, 1, :, :].to(self.device)
                    test_mask_lb3 = test_masks[:, None, 2, :, :].to(self.device)

                    test_predmask_lb1 = test_pred_masks[:, None, 0, :, :].to(self.device)
                    test_predmask_lb2 = test_pred_masks[:, None, 1, :, :].to(self.device)
                    test_predmask_lb3 = test_pred_masks[:, None, 2, :, :].to(self.device)
                    ...
                    vs_lb1 = compute_vs(test_predmask_lb1, test_mask_lb1)
                    vs_lb2 = compute_vs(test_predmask_lb2, test_mask_lb2)
                    vs_lb3 = compute_vs(test_predmask_lb3, test_mask_lb3)
